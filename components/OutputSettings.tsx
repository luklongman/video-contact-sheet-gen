...
import { Image, Settings, Download, Film } from 'lucide-react';
...
export default function OutputSettings() {
  ...
  const validateOutput = useCallback(() => {
    ...
    return isValidFormat && isValidQuality;
  }, [output]);

  // Validate whenever output changes
  useEffect(() => {
    const isValid = validateOutput();
    setStepCompleted(4, !!isValid);
  }, [output, validateOutput, setStepCompleted]);
    ...
    <Card className="w-full">
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Output & Generate</h2>
            <p className="text-sm text-gray-600">Configure output format and generate contact sheet</p>
          </div>
        </div>
        ...
        {!validateOutput() && (
          <Alert color="failure">
            <span className="font-medium">Invalid settings!</span> Please check your output settings. Quality must be between 1 and 100.
          </Alert>
        )}
        ...
        <div className="bg-blue-50 rounded-lg p-4">
          ...
          <Button
            ...
            disabled={!canGenerate || !validateOutput()}
          >
            {isProcessing ? (
              <>
        ...
        {processingError && (
          <Alert color="failure">
            <span className="font-medium">Processing failed!</span> {processingError}
          </Alert>
        )}
